...
/datum/controller/subsystem/processing/quirks/Initialize()
	if(!quirks.len)
		...
		// Раз изначально сделали через названия квирков, то типизируем хоть через ::name
		quirk_blacklist = list(
						  list(/datum/quirk/blindness::name,			/datum/quirk/nearsighted::name),
						  list(/datum/quirk/jolly::name,				/datum/quirk/depression::name,		/datum/quirk/apathetic::name),
						  list(/datum/quirk/no_taste::name,				/datum/quirk/deviant_tastes::name),
						  list(/datum/quirk/pineapple_liker::name,		/datum/quirk/pineapple_hater::name),
						  list(/datum/quirk/alcohol_tolerance::name,	/datum/quirk/alcohol_intolerance::name),
						  list(/datum/quirk/alcohol_intolerance::name,	/datum/quirk/drunkhealing::name),
						  list(/datum/quirk/asiat::name, 				/datum/quirk/ukraine::name),
						  list(/datum/quirk/no_guns::name,				"Ужасный стрелок"),
						  list(/datum/quirk/less_nightmare::name, 		/datum/quirk/lightless::name),
						  list(/datum/quirk/less_nightmare::name, 		/datum/quirk/nyctophobia::name),
						  list(/datum/quirk/less_nightmare::name, 		/datum/quirk/restorative_metabolism::name),
						  list(/datum/quirk/hallowed::name,				/datum/quirk/cursed_blood::name),
						  list(/datum/quirk/hallowed::name,				/datum/quirk/bloodfledge::name),
						  list(/datum/quirk/thirsty::name,				/datum/quirk/bloodfledge::name),
						  list(/datum/quirk/thirsty::name,				/datum/quirk/incubus::name),
						  list(/datum/quirk/thirsty::name,				/datum/quirk/succubus::name),
						  list(/datum/quirk/thirsty::name, 				/datum/quirk/powersaving::name),
						  list(/datum/quirk/thirsty::name, 				/datum/quirk/coolant_generator::name),
						  ...
						  list(/datum/quirk/syscleaner::name, 			/datum/quirk/restorative_metabolism::name),
						  list(/datum/quirk/restorative_nanobots::name, /datum/quirk/restorative_metabolism::name),
						  list(/datum/quirk/water_vulnerability::name, 	/datum/quirk/bluemoon_shower_need::name),
