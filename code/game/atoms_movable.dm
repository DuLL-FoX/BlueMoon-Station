...
/atom/movable/Destroy(force)
	// float() заводит анимацию с loop = -1 (см. /atom/movable/proc/float ниже), и
	// BYOND держит атом собственной внутренней ссылкой, пока анимация не кончилась.
	// Ни один DM-скан её не видит: рефтрекер показывает "внешних ссылок: 1" и пустой
	// контекст. В раунде 9972 это 56 хардделов из 150 и 8.6 секунды заморозки мира -
	// всё, что попало в невесомость и там умерло (люди, обезьяны, мозги). Гасим
	// НАСТОЯЩЕЙ нулевой анимацией: голый ANIMATION_END_NOW без time = 0 - это no-op.
	// alpha, а не pixel_z/pixel_y: их правят и float(), и movetype_handler, и
	// живое движение, а alpha не трогает никто. У госта та же правка стоит с
	// прошлого паса (observer.dm): он заводит свою анимацию мимо float(), поэтому
	// флага у него нет и гасит он безусловно.
	//
	// Назначаем alpha САМОЙ СЕБЕ, а не нулю: time = 0 применяется мгновенно, то
	// есть ноль - это настоящая прозрачность, а не "анимируем к нулю". Удаляемому
	// объекту она безразлична, но qdel без force на Destroy(), вернувшем
	// QDEL_HINT_LETMELIVE, откатывает удаление - и атом остался бы жить полностью
	// прозрачным. Присваивание текущего значения гасит анимацию ровно так же и
	// ничего не меняет в картинке.
	//
	// Гейт по флагу, а не безусловный вызов: Destroy проходит каждый движимый объект,
	// а FLOATING ставится и снимается ровно вместе с этой анимацией - других
	// бесконечных анимаций в дереве нет. Заодно ни один обычный qdel не получает
	// лишнего касания аппиранса.
