...
/// Usage тика, при котором ответ доехал в хвост занятого тика, а не по сети: МК уже выбрал
/// свой бюджет, и верб исполнился только когда интерпретатор освободился. Проверять это
/// приходится отдельно от CLIENT_LATENCY_SERVER_STALL_WARN_MS - тиковые часы замера считают
/// TICK_USAGE_REAL и растут вместе со стенными, так что разность двух часов такую задержку
/// не видит в принципе и спайк уезжал в «очередь клиента или транспорт».
#define CLIENT_LATENCY_MC_TAIL_TICK_USAGE TICK_LIMIT_MC
