...
/// Потолок на размер одной картинки фона лобби.
///
/// Фон уходит клиенту browse()-ом ПО ИГРОВОМУ СОЕДИНЕНИЮ, отдельной копией каждому:
/// мимо CDN, мимо дедупликации asset-кэша и мимо любого учёта. 15 августа в каталоге
/// лежал анимированный GIF на 23.9 МБ, и он же стоял загрузочным фоном - то есть его
/// получал КАЖДЫЙ, кто подключился, в том числе пока мир ещё инициализировался. Волна
/// реконнекта после рестарта - это сотня клиентов за минуту, то есть около 2.5 ГБ,
/// поставленных в исходящую очередь 32-битного DreamDaemon. Ровно так и выглядят
/// "сильные проблемы в сетевом потоке": фон не доезжает (белый экран лобби), клиентов
/// рвёт по таймауту, а процессу нечем платить за буферы.
///
/// Потолок намеренно жёсткий: 4 МБ на фон - это уже много, а всё, что больше, стоит
/// пережать. Отсечённые файлы называются в логе поимённо, чтобы их было видно сразу.
