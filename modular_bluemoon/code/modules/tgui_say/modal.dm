...
/**
 * Уводит ввод в панель, если игрок её выбрал.
 *
 * Вербы речи зовут это первой строкой и выходят: текст придёт из панели, а
 * серверный диалог в этом режиме не нужен вовсе. Иначе выбор способа ввода
 * работал бы только для горячих клавиш, а из меню и с кнопок всё равно
 * открывался бы старый диалог.
 */
/mob/proc/open_say_panel(channel)
	if(!client?.tgui_say_ready())
		return FALSE
	return client.tgui_say_open(channel)
